<?php
	if(isset($_POST['flag'])){
		...
		# flag check
		$p = $pdo->prepare('SELECT no, title, score FROM solveme_problem WHERE flag=:flag LIMIT 1');
		$p->bindValue(':flag', secure_hash($_POST['flag']));
		$p->execute();
		$prob = $p->fetch(PDO::FETCH_ASSOC) or die(json_encode(array('status' => 'x')));

		# is already solved
		$p=$pdo->prepare('SELECT 1 FROM solveme_authlog WHERE problem_no=:problem_no AND username=:username LIMIT 1');
		...
		# insert auth log
		$p=$pdo->prepare("INSERT INTO solveme_authlog(problem_no, username) VALUES(:problem_no, :username)");
		$p->bindParam(':problem_no', $prob['no']);
		$p->bindParam(':username', $_SESSION['username']);
		$p->execute();

		# update score
		$p=$pdo->prepare('UPDATE solveme_user SET score=score+:score WHERE username=:username LIMIT 1');
